...
def get_prompt_context(project_root: Path) -> str | None:
    ...
    if meta:
        try:
            from core.protocol.metacognition_compat import normalize_metacognition
    ...
    if goals:
        try:
            from core.protocol.goals_compat import extract_primary_goal_text
    ...
    if isinstance(epistemic, dict) and epistemic:
        try:
            from core.protocol.epistemic_summary import (
    ...
    if isinstance(outcomes, dict) and outcomes:
        try:
            from core.protocol.outcomes_summary import (
