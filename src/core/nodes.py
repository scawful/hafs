...
@dataclass
class HalextNode(ComputeNode):
    """Represents a Halext-specific AI node with gateway authentication."""

    api_token: Optional[str] = None
    org_id: Optional[str] = None
    gateway_url: Optional[str] = None
    node_type: str = "halext"
    capabilities: list[str] = field(default_factory=lambda: ["ollama", "halext", "remote-gpu"])
    ...
    async def load_config(self, config_path: Optional[Path] = None) -> int:
        ...
        try:
            ...
            for node_config in nodes:
                ...
                # Special handling for Halext nodes
                if node.node_type == "halext" or node_config.get("gateway_url"):
                    node = HalextNode(
                        **node.to_dict(),
                        api_token=node_config.get("api_token") or os.environ.get("HALEXT_API_KEY"),
                        org_id=node_config.get("org_id"),
                        gateway_url=node_config.get("gateway_url")
                    )
