...
class ServiceManager:
    ...
    def _create_adapter(self) -> ServiceAdapter:
        ...
        if system == "Darwin":
            from services.adapters.launchd import LaunchdAdapter

            return LaunchdAdapter()
        elif system == "Linux":
            from services.adapters.systemd import SystemdAdapter
    ...
    def _get_builtin_services(self) -> dict[str, ServiceDefinition]:
        ...
        return {
            ...
            "autonomy-daemon": ServiceDefinition(
                ...
                command=[python, "-m", "services.autonomy_daemon", "--interval", "30"],
                working_directory=repo_root,
                environment=environment,
            ),
            ...
            "context-agent-daemon": ServiceDefinition(
                ...
                command=[python, "-m", "services.context_agent_daemon", "--interval", "300"],
                working_directory=repo_root,
                environment=environment,
            ),
            "observability-daemon": ServiceDefinition(
                ...
                command=[python, "-m", "services.observability_daemon"],
                working_directory=repo_root,
                environment=environment,
            ),
