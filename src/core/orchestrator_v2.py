...
class UnifiedOrchestrator:
    ...
    OLLAMA_MODELS = {
        # Coding models
        "qwen2.5-coder:14b": "Qwen 2.5 Coder 14B - Best balanced coding model",
        "qwen2.5-coder:32b": "Qwen 2.5 Coder 32B - Powerful coding specialist",
        "deepseek-coder-v2-lite": "DeepSeek Coder V2 Lite - MoE coding",
        # General models
        "qwen3:14b": "Qwen3 14B - Best general local flagship",
        "deepseek-r1:14b": "DeepSeek R1 14B - Excellent reasoning",
        "gemma3:27b": "Gemma 3 27B - Heavyweight local reasoning",
        "llama3.3:70b": "Llama 3.3 70B - Frontier-class local model",
        # Embedding models
    ...
    DEFAULT_CONFIGS = {
        ...
        Provider.HALEXT: ProviderConfig(
            ...
            default_model="qwen3:14b-reasoning", # Prefer reasoning models for gateway
            priority=45,
            cost_per_1k_tokens=0.0,
    ...
    TIER_ROUTES = {
        ...
        TaskTier.RESEARCH: [
            (Provider.GEMINI, "gemini-3-pro-preview"),  # Deep thinking, 2M context
            (Provider.GEMINI, "gemini-3-flash-preview"),  # 1M context
        ...
        TaskTier.LOCAL: [
            ...
            (Provider.OLLAMA, "qwen2.5-coder:32b"),  # Best for 32GB+ GPU
            (Provider.OLLAMA, "qwen3:14b"),           # Best general local flagship
            (Provider.OLLAMA, "llama3.3:70b"),       # Best overall reasoning
            (Provider.OLLAMA, "deepseek-r1:14b"),     # Excellent reasoning
            (Provider.OLLAMA, "gemma3:27b"),         # Strong reasoning
            (Provider.OLLAMA, "qwen2.5-coder:14b"),  # Balanced coding
        ],
        TaskTier.CHEAP: [
    ...
    OLLAMA_MODEL_SELECTION = {
        "coding": "qwen2.5-coder:14b",  # Best balanced code model
        "fast": "qwen3:14b",           # Fast local flagship
        "reasoning": "deepseek-r1:14b", # Best balanced local reasoning
        "general": "qwen3:14b",         # Best general purpose
        "fallback": "qwen3:14b",        # Reliable fallback
    }
    ...
    async def _generate_with_provider(
        ...
        if provider == Provider.GEMINI:
            ...
            api_key = os.environ.get("HALEXT_API_KEY")
            org_id = os.environ.get("HALEXT_ORG_ID")
            base_url = os.environ.get("HALEXT_BASE_URL")
            
            # Prefer node metadata if available
            from core.nodes import HalextNode
            if self._node_manager:
                halext_nodes = [n for n in self._node_manager.nodes if isinstance(n, HalextNode)]
                if halext_nodes:
                    node = halext_nodes[0]
                    api_key = api_key or node.api_token
                    org_id = org_id or node.org_id
                    base_url = base_url or node.gateway_url
            
            backend = HalextBackend(
                base_url=base_url,
                api_key=api_key,
                org_id=org_id,
                model=model,
    ...
    async def _generate_halext(
        ...
        if not self._halext_client:
            from integrations.halext_client import HalextOrgClient
    ...
    async def load_custom_model(self, model_path: str, node_name: Optional[str] = None):
        """Load a custom model on a specific node.
        
        Args:
            model_path: Path to GGUF or Modelfile.
            node_name: Target node (optional).
        """
        if not self._node_manager:
            return
            
        node = self._node_manager.get_node(node_name) if node_name else None
        if not node:
            # Try to find a local GPU node
            node = await self._node_manager.get_best_node(prefer_gpu=True, prefer_local=True)
            
        if not node:
            logger.error("No node found for custom model loading")
            return
            
        logger.info(f"Loading custom model {model_path} on node {node.name}")
        # Support for GGUF/Modelfile loading via Ollama or Llama.cpp
