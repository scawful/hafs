...
Usage:
    # Run directly
    python -m services.context_agent_daemon

    # With options
    python -m services.context_agent_daemon --interval 3600

    # Install as launchd service
    python -m services.context_agent_daemon --install

    # Check status
    python -m services.context_agent_daemon --status
"""
...
class ContextAgentDaemon:
    ...
    async def _ensure_oracle_kb_builder(self):
        ...
        if self._oracle_kb_builder is None:
            try:
                from agents.knowledge.oracle import OracleKBBuilder
    ...
    async def _ensure_report_manager(self):
        ...
        if self._report_manager is None:
            try:
                from agents.utility.report_manager import ReportManager
...
<dict>
    <key>Label</key>
    <string>com.context-agent-daemon</string>
    <key>ProgramArguments</key>
    <array>
        <string>{python_path}</string>
        <string>-m</string>
        <string>services.context_agent_daemon</string>
...
"""
    plist_path = Path.home() / "Library" / "LaunchAgents" / "com.context-agent-daemon.plist"
    ...
    print("  launchctl list | grep context")
