...
from textual.widgets import Label, Markdown, Static, TextArea, LoadingIndicator

from models.afs import ContextRoot, MountPoint, MountType
from tui.utils.file_ops import can_edit_file, duplicate_file, read_text_file, rename_path
...
class ContextViewer(Widget):
    ...
    # Reactive state for async loading
    ...
    def _compose_file_view(self) -> ComposeResult:
        ...
        if is_text:
            ...
            if content is None:
                # Should be caught by loading state, but safety check
                return
    ...
    def set_file(self, path: Path) -> None:
        ...
        # Start async load
        self.run_worker(self._load_file_content(path), thread=True, exclusive=True)

    async def _load_file_content(self, path: Path) -> None:
        """Load file content in a background thread."""
        self.loading = True
        try:
            if can_edit_file(path):
                self._content = read_text_file(path)
            else:
                # Binary or other - we handle binary info in compose
                self._content = "" 
        except Exception as e:
