    ...
    def on_tree_node_expanded(self, event: Tree.NodeExpanded) -> None:
        ...
        if isinstance(data, dict) and data.get("type") == "global_context":
            ...
            try:
                ...
                from core.afs.discovery import load_context_root
...
class ExplorerWidget(Vertical):
    ...
    def on_tree_node_highlighted(self, event: Tree.NodeHighlighted) -> None:
        """Preview items as the cursor moves (debounced)."""
        event.stop()
        
        # Debounce highlight to avoid rapid file reads
        if hasattr(self, "_highlight_timer"):
            self._highlight_timer.stop()
        
        self._highlight_timer = self.set_timer(
            0.25, 
            lambda: self._handle_tree_data(event.node.data),
        )
