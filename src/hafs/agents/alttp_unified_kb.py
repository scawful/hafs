...
class BatchEmbeddingManager:
    ...
    def __init__(
        ...
        self._index: dict[str, str] = {}  # item_id -> embedding_file
        self._load_index()
    ...
    def _save_checkpoint(self, checkpoint: EmbeddingCheckpoint):
        """Save checkpoint."""
    ...
    async def generate_embeddings(
        self,
        items: list[tuple[str, str]],  # (item_id, text)
        kb_name: str,
        ...
        if checkpoint and checkpoint.kb_name == kb_name:
            ...
            for i, (item_id, _) in enumerate(items):
                if item_id == checkpoint.last_item_id:
                    start_index = i + 1
                    break
            logger.info(f"Resuming from checkpoint: {start_index}/{len(items)}")

        # Filter out already embedded items
        pending = [
            (item_id, text) for item_id, text in items[start_index:]
        ...
        logger.info(f"Generating embeddings for {len(pending)} items (batch size: {self.batch_size})")
...
class OracleOfSecretsKB(BaseAgent):
    ...
    async def setup(self):
        ...
        self._embedding_manager = BatchEmbeddingManager(
            self.kb_dir, self._orchestrator, batch_size=50
        )
...
class UnifiedALTTPKnowledge(BaseAgent):
    ...
    async def run_task(self, task: str = "stats") -> dict[str, Any]:
        """Run a unified KB task."""
        if task == "build":
            return await self.build_all()
            ...
            symbol = task[8:].strip()
            return await self.compare(symbol)

        elif task == "modifications":
            return {"modifications": await self.get_hack_modifications()}
