...
Example:
    mission = ResearchMission(
        mission_id="alttp_sprite_analysis",
        objective="Analyze sprite behavior patterns in ALTTP",
        kb_projects=["alttp"],
        research_queries=["sprite movement", "collision detection"],
    )

    agent = ALTTPResearchAgent(mission)
    await agent.setup()
...
from hafs.agents.autonomy_agents import MemoryAwareAgent, LoopReport
...
class GigaleakAnalysisAgent(MissionAgent):
    ...
    async def run_task(self) -> LoopReport:
        ...
        alttp_kb = await self.load_knowledge_base("alttp")
