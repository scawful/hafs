...
5. How it fits into the larger game loop"""
    ...
    async def explain_symbol(self, symbol_name: str) -> str:
        ...
        prompt = f"""Explain this ALTTP memory symbol:
...
5. Common patterns for reading/writing this address"""
    ...
    def get_statistics(self) -> dict[str, Any]:
        ...
        return {
            ...
            "symbols_by_category": categories,
            "routines_by_bank": banks,
        }

    async def run_task(self, task: str = "stats") -> dict[str, Any]:
        """Run a knowledge base task.
        ...
        Args:
            task: Task to run:
                ...
                - "search:QUERY" - Search
                - "analyze:ROUTINE" - Analyze routine
        ...
        if task == "build":
            ...
            analysis = await self.analyze_routine(routine)
            return {"routine": routine, "analysis": analysis}

        elif task.startswith("explain:"):
            symbol = task[8:].strip()
            explanation = await self.explain_symbol(symbol)
            return {"symbol": symbol, "explanation": explanation}

        else:
            return {
                "error": "Unknown task",
                "usage": ["build", "stats", "search:QUERY", "analyze:ROUTINE", "explain:SYMBOL"],
            }
