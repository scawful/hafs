...
def index(limit: int = typer.Option(200, help="Max new entries to embed")) -> None:
    """Index new history entries into embeddings."""
    from hafs.core.history import HistoryEmbeddingIndex

    async def _index() -> None:
        config = load_config()
        idx = HistoryEmbeddingIndex(config.general.context_root)
...
def summarize(
    ...
    async def _summarize() -> None:
        config = load_config()
        idx = HistorySessionSummaryIndex(config.general.context_root)
        if session_id:
